...
function baocun(a){
	var $tr = $(a).parents("tr");  // = parent().parent().parent()
	var goodsid = $tr.children("td:eq(0)").children("input").val();  //获取商品id
	var name = $tr.children("td:eq(2)").children("input").val();     //商品名称
	var spec = $tr.children("td:eq(3)").children("input").val();     //商品型号
	var price = $tr.children("td:eq(4)").children("input").val();     //商品价格
...
function xiugai(a){
	var $tr = $(a).parents("tr");  //取得当前行对象
	
	//对tr中 中间的几个可以修改的单元格进行文本框修改
	var $td3 = $tr.children("td:eq(2)");  //取得第三个单元格
	var $td4 = $($tr.children("td")[3]);  //取得第四个单元格
	var $td5 = $tr.children("td:eq(4)");  //取得第五个单元格
	var $td6 = $tr.children("td:eq(5)");  //取得第六个单元格
	editTd($td3);
	editTd($td4);
	editTd($td5);
	editTd($td6);
...
function  editTd($td){
	...
	$input.css("width","60px").css("height","25px");
	$td.html($input);
}
